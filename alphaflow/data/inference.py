...
class AlphaFoldCSVDataset:
    def __init__(
        self, config, path, data_dir=None, mmcif_dir=None, msa_dir=None, templates=False
    ):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        msa_features = self.data_pipeline._process_msa_feats(
            f"{self.msa_dir}/{msa_id}", item.seqres, alignment_index=None
        )
        data = {**mmcif_feats, **msa_features}

        feats = self.feature_pipeline.process_features(data, mode="predict")
        if self.templates:
            feats["extra_all_atom_positions"] = torch.from_numpy(
        ...
        if self.mmcif_dir is not None:
            ...
            with open(f"{self.mmcif_dir}/{pdb_id[1:3]}/{pdb_id}.cif") as f:
                feats["ref_prot"] = protein.from_mmcif_string(
                    f.read(), chain, name=item.name
                )
...
class CSVDataset:
    def __init__(
        self, config, path, mmcif_dir=None, data_dir=None, msa_dir=None, templates=False
    ):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.templates:
            ...
            mmcif_feats = dict(np.load(path, allow_pickle=True))
            extra_all_atom_positions = mmcif_feats["all_atom_positions"][0]
            batch["extra_all_atom_positions"] = torch.from_numpy(
                extra_all_atom_positions
            )
