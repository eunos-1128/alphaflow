...
class OpenFoldSingleDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.subsample_pos:
            ...
            mmcif_feats["all_atom_positions"] = mmcif_feats["all_atom_positions"][
                conf_idx
            ]
        elif self.num_confs:
            mmcif_feats["all_atom_positions"] = mmcif_feats["all_atom_positions"][
        ...
        msa_features = self.data_pipeline._process_msa_feats(
            f"{self.alignment_dir}/{item.msa_id}", item.seqres, alignment_index=None
        )
...
class OpenFoldBatchCollator:
    def __call__(self, prots):
        stack_fn = (
            lambda x: torch.stack(x, dim=0) if isinstance(x[0], torch.Tensor) else x
        )
